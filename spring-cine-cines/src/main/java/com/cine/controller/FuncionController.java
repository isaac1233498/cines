...
public class FuncionController {
	...
	public ResponseEntity<Object> registrar(@Valid @RequestBody Funcion funcion) {
		...
		URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}")
				.buildAndExpand(funcion.getIdFuncion()).toUri();
		return ResponseEntity.created(location).build();
	}

	@PutMapping
	public ResponseEntity<Funcion> modificar(@Valid @RequestBody Funcion funcion) {
		Funcion obj = service.modificar(funcion);
		return new ResponseEntity<Funcion>(obj, HttpStatus.OK);
	}
